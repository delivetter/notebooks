...
jornada_laboral = 240 -  temps_depot_hub - temps_descarrega_hub #min
...
def define_loading_unloading_points(filename):
    ...
    for i in range(1,len(G.nodes())+1):
        if "G"+str(i) not in graf_peatonal:
...
def define_delivery_points(filename):
    ...
    with open(file_path, 'r') as file:
        next(file)  # Skip header
        for line in file:
            entregues=assign_deliveries("ZELERIS")[i]
            divisio=1
            while entregues > 0:
...
def VRP_cid(G,dicti,D):
    ...
    G.add_edge("Source", "G63", cost=0 )
    
    G.add_edge("G63", "Sink", cost=0  )
    ...
    for node in GVRP.nodes:
        ...
        if node!="Source":
            ...
            GVRP.add_edge("Source", node, cost=nx.shortest_path_length(G, "G63",orignode, weight="cost"))
            GVRP.edges["Source",node]["time"] = GVRP.edges["Source",node]["cost"]/veh_speed + cid_time[node]
        if node!="Sink":
            if node[0] != "S": orignode=node[:node.find('.')]
            GVRP.add_edge(node, "Sink", cost=nx.shortest_path_length(G, orignode,"G63", weight="cost"))
...
punts_cid = define_loading_unloading_points("GrafVial_Nodes_10mC.csv")
...
#VRP for VEHICLES
GVRP= VRP_cid(graf_peatonal,assignacio,punts_delivery) #canvi per graf_vial
...
for ruta in routes:
    ...
    for i in range(0,len(routes[ruta])-1):
        ...
        dibuixar(graf_peatonal, node1,node2,node_colors[ruta])
