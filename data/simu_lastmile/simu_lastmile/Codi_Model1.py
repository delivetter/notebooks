...
#Cx -> punt de descarrega

#Entregues diaries per operador
entreguesoperador={"CORREOS":166,"AMAZON":150,"DHL":77,"UPS":34,"GLS":36,"ZELERIS":35}

#Operador a simular
operador_escollit = "ZELERIS"
...
vehicle_cap=200 #parcels

jornada_laboral = 240 #min

veh_speed=166.67 #10 km/h --> 166.67 m/min
...
del_time = 2 #min/parcel

park_time = 2 #min/stop in cid



#NUMBER OF PACKAGES THAT AN OPERATOR CAN HAVE SIMULTANEOUSLY
operator_cap=10 #parcels
...
def create_graf_vial(filename):
    ...
    #ELIMINEM NODES V QUE NO ESTIGUIN CONNECTATS
...
def define_loading_unloading_points(filename):
    #Get loading and unloading points from the file
    G = nx.DiGraph()
    i=1
    file_path = filename
    with open(file_path, 'r') as file:
...
def define_delivery_points(filename):
    #Fet delivery points from the file
    G = nx.DiGraph()
    i=1
    file_path = filename
    with open(file_path, 'r') as file:
        next(file)  # Skip header
        for line in file:
            entregues=assign_deliveries(operador_escollit)[i]
            divisio=1
            while entregues > 0:
...
def divide_cid_assignment(G_del, assignacio, vehicle_cap):
    #In case that a CiD point has more deliveries assigned than the operator's capacity, division of
    #the CiD are created.
...
def VRP_cid(G,dicti,D):
    '''
    Fem el VRP des del Depot fins als punts de descarrega
    '''
    ...
    GVRP.add_node("Source", coords=(428070, 4576604))
    GVRP.add_node("Sink", coords=(428070, 4576604))

    G.add_node("Source", coords=(428070, 4576604))
    G.add_node("Sink", coords=(428070, 4576604))
    ...
    G.add_edge("Source", "V1", cost=cost_to_depot )
    
    G.add_edge("V1", "Sink", cost=cost_to_depot  )
    ...
    for node in GVRP.nodes:
        ...
        if node!="Source":
            ...
            #print(distance(G.nodes["Source"],G.nodes[orignode]))
            GVRP.add_edge("Source", node, cost=7300 + distance(G.nodes["V1"],G.nodes[orignode])*1.5)
            GVRP.edges["Source",node]["time"] = GVRP.edges["Source",node]["cost"]/veh_speed_depot + cid_time[node]
        if node!="Sink":
            if node[0] != "S": orignode=node[:node.find('.')]
            GVRP.add_edge(node, "Sink", cost=7300+distance(G.nodes["V1"],G.nodes[orignode])*1.5)
            GVRP.edges[node,"Sink"]["time"] = GVRP.edges[node,"Sink"]["cost"]/veh_speed_depot
...
def VRP_delivery(cid,division,G,dicti,D):
    '''
    Fem el VRP des del punt de descarrega fins a cada delivery point
...
punts_cid = define_loading_unloading_points("Punts_CiDC.csv")
...
for cid in all_routes:
    for division in all_routes[cid]:
        ...
        cid_time[cid+"."+str(division)]= trip_time + all_dists[cid][division]/op_speed + park_time
...
import openpyxl
file_path = '/Users/miguel/Desktop/MODELS BOTNET/MODEL ACTUAL/resultats_bau.xlsx'
...
output_list=[]
output_list.append("Model BaU. It. "+str(ii))
output_list.append(seed_num)
output_list.append(operador_escollit)
output_list.append(entreguesoperador[operador_escollit])
output_list.append(van_total_distance)
output_list.append(op_total_distance)
output_list.append(len(routes))


print("Distancia total a peu dels operaris: "+str(op_total_distance))
print("Distancia recorreguda per les furgonetes: " + str(van_total_distance))
...
for trip in routes:
    ...
    output_list.append(suma/60)
    print("______________________________________________________________")

row_number = ii+1
for index, value in enumerate(output_list, start=1):
...
nx.draw(graf_vial,get_position_dict(graf_vial),node_size=5, edge_color="grey", node_color="grey",with_labels=False) #malla vial

for ruta in routes:
    print(routes[ruta])
    for i in range(1,len(routes[ruta])-1):
        if routes[ruta][i][0]=="S": continue
        ...
        dibuixar(graf_vial, node1,node2,clr[ruta])
